      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="text-2xl font-black text-slate-950">أقرب الحجوزات</h2>
              <p className="text-sm font-bold text-slate-500">لمحة سريعة على اللي جاي حتى تبدأ يومك من أول نظرة.</p>
            </div>
            <div className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-black ${doctor?.is_available ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}>
              <Stethoscope className="h-4 w-4" />
              {doctor?.is_available ? "العيادة متاحة الآن" : "العيادة مغلقة الآن"}
            </div>
          </div>
          <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            {upcomingAppointments.length === 0 ? (
              <div className="col-span-full rounded-2xl border border-dashed border-slate-300 p-6 text-center text-sm font-semibold text-slate-500">
              ...
              upcomingAppointments.map((item) => (
                <article key={item.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-xs font-bold text-slate-400">{item.time || "بدون وقت"}</span>
                  </div>
                  <h3 className="mt-3 text-lg font-black text-slate-950">{item.patient_full_name || item.patient_name}</h3>
                  <p className="mt-1 text-sm font-semibold text-slate-500">{item.patient_phone}</p>
                  <p className="mt-3 text-xs font-bold text-slate-400">{STATUS_LABELS[item.status] || item.status}</p>
                </article>
              ))
            )}
          </div>
