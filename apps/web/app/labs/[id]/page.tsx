...
export async function generateStaticParams() {
  const services = await getMedicalServices("lab");
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  return {
    title: service ? `${service.name} | المختبرات الطبية | ملامح` : "مختبر طبي | ملامح",
    description: service?.description ?? "تفاصيل المختبر على منصة ملامح.",
  };
}
      ...
      <section className="page-hero-dark relative overflow-hidden text-white">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 pt-12 pb-20">
          ...
          <div className="flex items-center gap-2 text-xs text-slate-400 mb-8 font-black">
            ...
            <span>/</span>
            <Link href="/labs" className="hover:text-white transition-colors">المختبرات الطبية</Link>
            <span>/</span>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-5">
            ...
            <div className="bg-gradient-to-br from-slate-950 to-slate-900 rounded-3xl p-6 text-white">
              <p className="text-sm font-black mb-1">هل أنت صاحب المختبر؟</p>
              <p className="text-slate-400 text-xs font-semibold mb-4 leading-6">سجّل مختبرك واعرض خدماتك لآلاف المرضى يومياً.</p>
              <Link
                href="/join"
