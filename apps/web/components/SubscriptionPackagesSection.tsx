    ...
    <section className={`${compact ? "py-10" : "py-16"} px-4`} dir="rtl">
      <div className="mx-auto max-w-6xl">
        <div className="mb-8 text-right">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-primary/5 border border-primary/15 px-3 py-1 text-xs font-black text-primary">
            <Sparkles className="h-3.5 w-3.5" />
            للأطباء والعيادات
          </span>
          <h2 className="mt-3 font-display text-2xl md:text-3xl font-bold text-slate-950">باقات الاشتراك — ماذا نقدّم؟</h2>
            ...
            return (
              <article
                ...
                className={`relative flex flex-col rounded-3xl border bg-white p-6 shadow-lg transition hover:-translate-y-1 ${
                  recommended ? "border-primary/40 ring-2 ring-primary/10" : "border-slate-200"
                }`}
              >
                {recommended ? (
                  <span className="absolute -top-3 left-4 inline-flex items-center gap-1 rounded-full bg-[#0c5e47] px-3 py-1 text-[10px] font-black text-white">
                ...
                <div className="mt-5 flex items-end gap-2">
                  <span className="text-3xl font-black text-primary">${pkg.price_usd}</span>
                ...
                {showCta ? (
                  <Link
                    ...
                    className={`mt-6 block rounded-2xl py-3 text-center text-sm font-black transition ${
                      recommended ? "bg-primary text-white hover:bg-primary-dark" : "bg-slate-900 text-white hover:bg-slate-800"
                    }`}
                  >
                    اشترك الآن
                  </Link>
