...
export default function DoctorLoginScreen() {
  ...
  const submit = async () => {
    setLoading(true);
    const { response, data } = await apiFetch("/api/doctor/login", {
      method: "POST",
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: "#f8fafc" }} contentContainerStyle={{ padding: 16, paddingTop: 48, paddingBottom: 120 }}>
      <AppCard>
        <AppTitle>دخول الطبيب</AppTitle>
        <AppSubtitle>فصلنا دخول الطبيب عن الأدمن حتى كل واحد يشوف لوحته الخاصة فقط.</AppSubtitle>
